...
twinkle.test1a = function(cluster = NULL){
	...
	sink(zz)
	print(data.frame(vdfpars = vdfpars, vdfse = vdfse))
	print(data.frame(tinklepars = twinklepars, twinklese = twinklese))
	...
	colnames(cfm) = models
	rownames(cfm) = c("s1.phi0", "s1.phi1", "s1.psi1", "s2.phi0", "s2.phi1", "s2.phi2", "s2.psi1", "psi1", "s1.c", "s1.alpha1","sigma")
...
twinkle.test1b = function(cluster = NULL){
	...
	colnames(cfm) = models
	rownames(cfm) = c("s1.phi0", "s1.phi1", "s1.psi1", "s2.phi0", "s2.phi1", "s2.phi2", "s2.psi1", "s1.c", "s1.alpha1","sigma","s1.sigma","s2.sigma","omega","alpha1","beta1","gamma1")
...
twinkle.test1h = function(cluster = NULL){
	...
	jpeg("test1h.jpeg", width=1200, height=1000, quality=100)
	VaRplot(0.05, as.xts(d1)[,3], as.xts(d1)[,2], title=paste("Weekly returns and Out-of-Sample STAR-Normal\nValue-at-Risk Exceedances\n","(alpha=", 0.05,")",sep=""),
			ylab = "Dutch Guilder Weekly Returns")	
	plot(as.xts(as.data.frame(roll1))[,6], main = "Prob[")
