...
.stars1sLLH = function(pars, arglist)
{
	...
	if(arglist$transform==1){
		...
		if(modelinc[27]>0 && ipars["sigma",4]==1) ipars["sigma",1] = logtransform(ipars["sigma",1], ipars["sigma","LB"], ipars["sigma","UB"])
		if(modelinc[38]>0 && ipars["skew",4]==1) ipars["skew",1] = logtransform(ipars["skew",1], ipars["skew","LB"], ipars["skew","UB"])
		if(modelinc[39]>0 && ipars["shape",4]==1) ipars["shape",1] = logtransform(ipars["shape",1], ipars["shape","LB"], ipars["shape","UB"])
		if(modelinc[40]>0 && ipars["ghlambda",4]==1) ipars["ghlambda",1] = logtransform(ipars["ghlambda",1], ipars["ghlambda","LB"], ipars["ghlambda","UB"])
	}
	trace = arglist$trace
	T = length(data)
...
.stars2sLLH = function(pars, arglist)
{
	...
	ans = switch(returnType,
			llh = llh,
			LHT = LHT,
			all = list(llh = llh, res = epsx, z = z, LHT = LHT, probs = probs, condm = matrix(ans$condm, ncol=2), 
					constm = matrix(ans$constm, ncol=2), ipars = ipars, pmu = pmu))
	return(ans)
}
...
.stars1dLLH = function(pars, arglist)
{
	...
	mvar = ifelse(arglist$recinit$type==1, mean(res[1:arglist$recinit$n]*res[1:arglist$recinit$n]), rugarch:::backcastv(res, T, arglist$recinit$n))
	if(modelinc[36]>0) {
		mv = sum(apply(matrix(vexdata, ncol = modelinc[36]), 2, "mean")*ipars[idx["vxreg",1]:idx["vxreg",2],1])
	} else{
		mv = 0
	}
	hEst = mvar
	...
	if(modelinc[47]==3){
		if(modelinc[28]==0){
			mvar2 = ifelse(modelinc[42]>=0, modelinc[42], mvar)
	...
	if(fit.control$stationarity == 1 && modelinc[36] == 0){
	...
	if(modelinc[36]>0) vexdata = as.double(as.vector(vexdata)) else vexdata = double(1)
...
{	
	...
	ans = try( .C("garchfilterC", model = as.integer(modelinc[1:60]), 
					pars = as.double(ipars[,1]), idx = as.integer(idx[,1]-1), 
					prob = as.double(probs), res = as.double(res), e = double(T), 
					z = double(T), vexdata = double(1), 
					hEst = double(1), h = double(T), nres = double(1), 
					meanz = double(1), m = as.integer(m), T = as.integer(T), 
					LHT = double(T), llh = double(1),  PACKAGE = "twinkle"), silent = TRUE )
	if(inherits(ans, "try-error")){
		if(arglist$pmode!=1){
			assign("twinkle_csol", 1, envir = starenv)
...
dstar2 = function(ipars, arglist)
{
	...
	initp = (cnst+sum(alpha * colMeans(XL[1:N,,drop=FALSE]))) /(1-beta)
	pmu = cnst + as.numeric(XL%*%alpha)
	if(modelinc[20]>0) pmu = .recfilter(as.double(pmu), as.double(beta), init = as.double(initp))
	probs[,1] = 1/(1+exp(-pmu))
	probs[,2] = 1 - probs[,1]
...
dstar3 = function(ipars, arglist)
{
	...
	initp1 = (cnst1+sum(alpha1 * colMeans(XL[1:N,,drop=FALSE])))/(1-beta1)
	initp2 = (cnst2+sum(alpha2 * colMeans(XL[1:N,,drop=FALSE])))/(1-beta2)
	pmu1 = cnst1 + as.numeric(XL%*%alpha1)
	pmu2 = cnst2 + as.numeric(XL%*%alpha2)
	if(modelinc[20]>0) pmu1 = .recfilter(as.double(pmu1), as.double(beta1), init = as.double(initp1))
	if(modelinc[23]>0) pmu2 = .recfilter(as.double(pmu2), as.double(beta2), init = as.double(initp2))
...
dstar4 = function(ipars, arglist)
{
	...
	initp1 = (cnst1+sum(alpha1 * colMeans(XL[1:N,,drop=FALSE]))) /(1-beta1)
	initp2 = (cnst2+sum(alpha2 * colMeans(XL[1:N,,drop=FALSE]))) /(1-beta2)
	initp3 = (cnst3+sum(alpha3 * colMeans(XL[1:N,,drop=FALSE]))) /(1-beta3)
	...
	if(modelinc[20]>0) pmu1 = .recfilter(as.double(pmu1), as.double(beta1), init = as.double(initp1))
	if(modelinc[23]>0) pmu2 = .recfilter(as.double(pmu2), as.double(beta2), init = as.double(initp2))
	if(modelinc[26]>0) pmu3 = .recfilter(as.double(pmu3), as.double(beta3), init = as.double(initp3))
