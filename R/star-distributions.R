# Imported from twinkle package

#################################################################################
##
##   R package twinkle by Alexios Ghalanos Copyright (C) 2008-2013.
...
# distributions used in the twinkle package. Since version 1.2-5, the 
...
dsghst = function(x, mu=0, sigma=1, skew=1, shape=8, log = FALSE)
{
	...
	sol = try(.C("c_dghst", x = as.double(x), mu = as.double(mu), 
					...
					logr = as.integer(log), PACKAGE="twinkle"), silent = TRUE)
...
psnorm = function(q, mu = 0, sigma = 1, skew = 1.5)
{
	...
	sol = try(.C("c_psnorm", q = as.double(q), mu = as.double(mu), 
					sigma = as.double(sigma), skew = as.double(skew), ans = ans, 
					n = as.integer(maxn), PACKAGE="twinkle"), silent = TRUE)
	if(inherits(sol, 'try-error')){
		return(sol)
	} else{
		return(sol$ans)
...
rged = function(n, mu = 0, sigma = 1, shape = 2)
{
	...
	sol = try(.C("c_rged", n = as.integer(n), mu = as.double(mu), 
					sigma = as.double(sigma), shape = as.double(shape), 
					ans = ans, PACKAGE="twinkle"), silent = TRUE)
	if(inherits(sol, 'try-error')){
		return(sol)
	} else{
		return(sol$ans)
...
{   
	...
	retValues <- .C("qNIG",
			...
			i_n = as.integer(n),
			q = as.double(.CArrange(q, 1, 1, n)), PACKAGE="twinkle")
